// Review 표의 Answers 열에 쓰는 답 값의 약어. 순수 함수라 화면 없이 테스트한다 (prototype 의 answerTokens.ts 와 같은 규칙).
...
function tokenOf(group, original) {
 ...
 * normalizeLabel 이 같은 값(`Not covered`와 `Not Covered`)은 한 값으로 보고 같은 토큰을 준다.
 */
export function abbreviate(values) {
  ...
  // 겹치는 토큰이 없어질 때까지 겹친 그룹의 글자 수를 늘린다. full 인 그룹은 원래 값이 서로 다르므로 언젠가 끝난다
  for (;;) {
 ...
 * 값이 여럿이면 normalizeLabel 이 같은 표기 차이다.
 */
export function legendEntries(tokens) {
  const byToken = new Map();
...
export function tokenWidthOf(tokens) {
  let longest = 1;
